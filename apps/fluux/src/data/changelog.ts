    ...
    sections: [
      {
        ...
        items: [
          ...
          'Search context preview with sender nick and multi-line snippets in sidebar results',
          'Keyboard navigation in search result sidebar',
          'Search index backfill to index pre-existing cached messages',
          'Quoted exact phrase search support (wrap terms in quotes)',
          'Search rebuild progress UI and empty state improvements',
          'Persistent search highlights with improved scrolling to matched messages',
          'Visual distinction between search preview and live chat view',
          'XEP-0359: Origin-id support for outgoing stanzas',
          'Poll settings for single/multiple choice and hidden results before voting',
      ...
      },
      {
        ...
        items: [
          ...
          'Prefer occupant nick over contact name in MUC room display resolution',
          ...
          'Close poll button now works for polls loaded from history',
          'Reaction replay no longer resets read state in activity log',
          'Reconnect backoff reset on wake from sleep',
          'Search scroll race conditions and ResizeObserver conflicts resolved',
          'Search input preserves spaces for multi-word queries',
          'Alt+number sidebar shortcuts now work from message input',
          'Cached MUC occupant avatars restored across sessions',
          'Restored scroll position bounds-checked to prevent stuck-at-top',
          'More-options dropdown no longer overlaps bottom of chat on last message',
          'Search index progress bar color corrected',
        ],
      },
    ],
  },
